			...
			if(preg_match('#^(([a-z0-9!\#$%&\\\'*+/=?^_`{|}~-]+\.?)*[a-z0-9!\#$%&\\\'*+/=?^_`{|}~-]+)@(([a-z0-9-_]+\.?)*[a-z0-9-_]+)\.[a-z]{2,}$#i', $_POST['email']))
			{
				...
				if($dn == 0)
				{
					...
					if(mysqli_query($link, 'insert into users(username, password, email, salt) values ("'.$username.'", "'.$password.'", "'.$email.'","'.$salt.'")'))
					{
						...
						$form = false;
?>
		<div class="message">You have successfuly been signed up. You can now <a href="login.php" style="color:white">Log in</a>
        </div>
<?php
					}
					else
...
if ($form) {
	...
	if(isset($message)) echo '<div class="message" style="background:red">'.$message.'</div>';
...
?>
		<div class="content">
			<form action="sign_up.php" method="post">
				<br />
				<div class="center">
					<label for="username">Username</label><input required=true type="text" name="username" value="<?php if(isset($_POST['username'])){echo htmlentities($_POST['username'], ENT_QUOTES, 'UTF-8');} ?>" /><br />
					<br />
					<label for="password">Password<span class="small">&nbsp;(min 10 characters)</span></label><input required=true type="password" name="password" /><br />
					<br />
					<label for="passverif">Confirm Password<span class="small"></span></label><input required=true type="password" name="passverif" /><br />
					<br />
					<label for="email">Email</label><input required=true type="text" name="email" value="<?php if(isset($_POST['email'])){echo htmlentities($_POST['email'], ENT_QUOTES, 'UTF-8');} ?>" /><br />
					<br />
